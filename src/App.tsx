...
import { GlobalTaskProvider, useGlobalTask } from './context/GlobalTaskManager';

function Navbar() {
  const { isGenerating } = useGlobalTask();
  return (
       ...
       <div className="flex items-center space-x-6">
          {isGenerating && (
             <div className="flex items-center text-yellow-500 text-xs font-mono bg-yellow-900/20 px-3 py-1 rounded-full border border-yellow-800 animate-pulse">
                <Loader2 className="mr-2 animate-spin" size={12}/> Generating Questions...
             </div>
          )}

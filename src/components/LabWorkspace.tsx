...
import {
  ...
  Play, Maximize2, Minimize2, Loader2, Save, FileText, FlaskConical
} from 'lucide-react';
...
export default function LabWorkspace({ subject }: { subject: Subject }) {
  ...
  const handleAiAssist = async (action: 'explain' | 'shorten' | 'comment') => {
      setLoadingAi(true);
      setAiResponse('');
      ...
      try {
          ...
          setAiResponse("AI Assistant failed to respond. Ensure backend is running.");
      } finally {
          setLoadingAi(false);
      }
  };
    ...
    <div className="flex h-[calc(100vh-6rem)] bg-gray-950 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col relative">
          ...
          <div className="flex-1 flex overflow-hidden">
              ...
              {/* AI Panel */}
              {aiPanelOpen && (
                  <div className="w-80 bg-gray-900 border-l border-gray-800 flex flex-col animate-in slide-in-from-right duration-300">
                      <div className="p-3 border-b border-gray-800 font-bold text-gray-300 text-xs uppercase tracking-wider flex items-center">
                          <Code size={14} className="mr-2"/> AI Assistant
                      </div>
                      <div className="flex-1 p-4 overflow-y-auto text-sm text-gray-300">
                          {loadingAi ? (
                              ...
                              <div className="text-center text-gray-600 mt-10">
                                  Select an action from the toolbar to get AI help.
                              </div>
                          )}
                      </div>
                  </div>
